...
for buffer_slice_idx in range(num_buffer_slices):
    ...
    # Packet Delay Line --------------------------------------------------------------------------
    comp_packet_delay_line = sst.Component("packet_delay_line_%d" % (buffer_slice_idx), "OSB_1_0.PacketDelayLine")
    comp_packet_delay_line.addParams({
        "idx" : buffer_slice_idx,
        "verbose" : 3,
    })
    packet_delay_lines.append(comp_packet_delay_line)

    # Buffer Port Mux --> data Packer
    link_buffer_port_mux_data_packer = sst.Link("link_buffer_port_mux_data_packer_%d" % (buffer_slice_idx))
    ...
    # Data Packer --> Packet Delay Line
    link_data_packer_packet_delay_line = sst.Link("link_data_packer_packet_delay_line_%d" % (buffer_slice_idx))
    link_data_packer_packet_delay_line.connect(
        (comp_data_packer, "out_packet_bus", "10ps"),
        (comp_packet_delay_line, "input1", "10ps"),
    )
    # ---------------------------------------------------------------------------------------------------
# ---------------------------------------------------------------------------------------------------
...
# Final Slice
comp_final_slice = sst.Component("final_slice", "OSB_1_0.FinalSlice")
comp_final_slice.addParams({
    "frequency" : clock_frequency,
    "buffer_slices" : num_buffer_slices,
    "port_group_interfaces" : num_pgs,
    "ports_per_buffer_slice" : ports_per_buffer_slice,
    "ports_per_pg" : num_ports_per_pg,
    "verbose" : 3,
})

link_lookup_pipeline_final_slice = sst.Link("link_lookup_pipeline_final_slice")
link_lookup_pipeline_final_slice.connect(
    (comp_lookup_pipeline, "out_fds_bus", "10ps"),
    (comp_final_slice, "fds_input", "10ps"),
)
...
for buffer_slice_idx in range(num_buffer_slices):
    ...
    # Packet Delay Line --> Receiver
    link_packet_delay_line_receiver = sst.Link("link_packet_delay_line_receiver_%d" % (buffer_slice_idx))
    link_packet_delay_line_receiver.connect(
        (packet_delay_lines[buffer_slice_idx], "output1", "10ps"),
