...
frontplane_config = "osb_1_0/config/frontplane_config/16x200G.json"

# Loading ports config file
ethernet_config = "osb_1_0/config/ethernet_config/4x200G.json"
...
# Model attributes ---------------------------------------------------------------------------------
num_pgs = 2
num_ports_per_pg = len(ports_config)
...
for buffer_slice_idx in range(num_buffer_slices):
    ...
    comp_packet_data_multi_fifo.addParams({
        ...
        "frontplane_config": frontplane_config,
        "req_enable": 1,
        "overflow": 0,
        "verbose" : 1,
    })
...
# Receiver -------------------------------------------------------------------------------------------
comp_receiver = sst.Component("receiver_port_group", "OSB_1_0.Receiver")
...
link_receiver_finisher = sst.Link("link_buffer_port_mux_receiver")
link_receiver_finisher.connect(
    (comp_buffer_port_mux, "pkt_out", "10ps"),
...
link_receiver_finisher = sst.Link("link_receiver_finisher")
link_receiver_finisher.connect(
    (comp_receiver, "output", "10ps"),
    (comp_finisher, "egress_port_0_0", "10ps"),
)
