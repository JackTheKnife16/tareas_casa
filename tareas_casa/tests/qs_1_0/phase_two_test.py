...
subcomp_traffic_profile = comp_packet_generator.setSubComponent("traffic_profile", "TRAFFIC_GEN_1_0.PhaseTwo", 0)
subcomp_traffic_profile.addParams({
    "frontplane_config": port_config_file,
    "packet_size_list": "1024, 1048, 1096, 1124",
...
for port in ports_config:
    ...
    link_packet_gen_ingress_port = sst.Link("link_packet_gen_ingress_port_%d_%d" % (0,i))
    link_packet_gen_ingress_port.connect(
        (comp_packet_generator, "ingress_port_%d_%d" % (0,i), "10ps"),
...
for port in ports_config:
    ...
    for j in range(8):
        ...
        subcomp_priority_queues.append(subcomp_pri_queue)
        # -----------------------------------------------------------------------------------------------
    ...
    link_egress_port_finisher.connect(
        ...
        (comp_finisher, "egress_port_%d_%d" % (0,i), "5ps"),
    )
...
# Egress ports
egress_ports_receiving_traffic = [0, 8]
# egress_ports_receiving_traffic = list(range(0, 52))
