...
traffic_config_file = "qs_1_0/config/traffic_config/traffic_config_ingress_port_fairness.json"
...
# Define SST core options
# sst.setProgramOption("stopAtCycle", "145us")
...
subcomp_traffic_profile = comp_packet_generator.setSubComponent("traffic_profile", "TRAFFIC_GEN_1_0.IngressDualQ", 0)
...
# Packet Classifier ----------------------------------------------------------------------------------
comp_packet_classifier = sst.Component("packet_classifier", "QS_1_0.DualQPacketClassifier")
...
for port in ports_config:
    ...
    # Ingress Port [i] --> Packet Classifer [i]
    link_ingress_port_packet_classifier = sst.Link("link_ingress_port_packet_classifier_%d" % (i))
    link_ingress_port_packet_classifier.connect(
        (comp_ingress_port, "acceptance_checker", "10ps"),
        (comp_packet_classifier, "in_%d" % (i), "10ps")
    )
    # ------------------------------------------------------------------------------------------------
    i += 1

# Acceptance Check ----------------------------------------------------------------------------------
comp_acceptance_checker = sst.Component("acceptance_checker", "QS_1_0.DualQAcceptanceCheckerV2")
...
# MEMORY ACCOUNTING ------------------------------------------------------------------------------------------
# Global Pool Util ----------------------------------------------------------------------------------------
comp_global_pool_util = sst.Component("global_pool_util", "QS_1_0.DualQGlobalPoolUtil")
...
for port in ports_config:
    ...
    # Port FIFOs Event Forwarder -----------------------------------------------------------------------------------------------
    comp_port_fifo_event_forwarder = sst.Component("port_fifo_event_forwarder_%d" % (i), "QS_1_0.EventForwarder")
    comp_port_fifo_event_forwarder.addParams({
        "num_inputs": 8,
        "verbose" : verbosity_option,
    })
    # ----------------------------------------------------------------------------------------------------
    ...
    # Scheduler [i] ------------------------------------------------------------------------------------
    comp_scheduler = sst.Component("scheduler_%d" % (i), "QS_1_0.DualQStrictPriSchedulerV2")
    ...
    for j in range(8):
        # Priority Queue [i][j] ------------------------------------------------------------------------
        subcomp_pri_queue = comp_port_fifos.setSubComponent("priority_queues", "QS_1_0.DualQPriorityQueue", j)
        ...
        # Connections -----------------------------------------------------------------------------------
        # Priority Queue[i][j] --> Port Event Forwarder
        link_pri_queue_event_forwarder = sst.Link("link_pri_queue_event_forwarder_%d_%d" % (i, j))
        link_pri_queue_event_forwarder.connect(
            (subcomp_pri_queue, "output", "5ps"),
            (comp_port_fifo_event_forwarder, "input_%d" % (j), "5ps"),
        )
        
        # Scheduler[i] --> Priority Queue[i][j]
        link_scheduler_pri_queue = sst.Link("link_scheduler_pri_queue_%d_%d" % (i, j))
        link_scheduler_pri_queue.connect(
    ...
    # L queue
    subcomp_l_queue = comp_l4s_port_fifos.setSubComponent("priority_queues", "QS_1_0.DualQPriorityQueue", 0)
    ...
    # Connections -----------------------------------------------------------------------------------
    # L Queue[i] <--> Acceptance Checker[i]
    link_l_queue_acceptance_checker = sst.Link("link_l_queue_acceptance_checker_%d_%d" % (i, j))
    link_l_queue_acceptance_checker.connect(
        (subcomp_l_queue, "output", "10ps"),
        (comp_acceptance_checker, "l_fifo_%d" % (i), "10ps"),
    )
    # Scheduler[i] --> L4S Priority Queue[i][j]
    link_scheduler_l4s_pri_queue = sst.Link("link_scheduler_l4s_pri_queue_%d_%d" % (i, j))
    link_scheduler_l4s_pri_queue.connect(
        (subcomp_l_queue, "scheduler", "5ps"),
        (comp_scheduler, "l_priority_queue", "5ps"),
    )

    # Port FIFO Event Forwarder [i] <--> Acceptance Checker[i]
    link_port_fifos_acceptance_checker = sst.Link("link_port_fifos_acceptance_checker_%d_%d" % (i, j))
    link_port_fifos_acceptance_checker.connect(
        (comp_port_fifo_event_forwarder, "output_0", "5ps"),
        (comp_acceptance_checker, "c_fifo_%d" % (i), "5ps"),
    )
    # --------------------------------------------------------------------------------------------------
    ...
    # Connections -----------------------------------------------------------------------------------------
    # Packet Classifier [i] <--> Port Fifo [i]
    link_packet_classifier_port_fifos = sst.Link("link_packet_classifier_port_fifos_%d" % (i))
    link_packet_classifier_port_fifos.connect(
        (comp_packet_classifier, "fifo_%d" % (i), "10ps"),
        (comp_port_fifos, "wred", "10ps"),
    )

    # Packet Classifier [i] <--> L4S Port Fifo [i]
    link_packet_classifier_l_port_fifos = sst.Link("link_packet_classifier_l_port_fifos_%d" % (i))
    link_packet_classifier_l_port_fifos.connect(
        (comp_packet_classifier, "l_fifo_%d" % (i), "10ps"),
        (comp_l4s_port_fifos, "wred", "10ps"),
    )

    # Classic WRED port fifo [i] <--> Scheduler [i]
    link_classic_wred_scheduler = sst.Link("link_classic_wred_scheduler_%d" % (i))
    link_classic_wred_scheduler.connect(
        (comp_classic_wred, "port_fifo_%d" % (i), "5ps"),
        (comp_scheduler, "packet_in", "5ps"),
    )

    # L4S WRED port fifo [i] <--> Scheduler [i]
    link_l4s_wred_scheduler = sst.Link("link_l4s_wred_scheduler_%d" % (i))
    link_l4s_wred_scheduler.connect(
        (comp_l4s_wred, "port_fifo_%d" % (i), "5ps"),
        (comp_scheduler, "l_packet_in", "5ps"),
    )
...
# MG Utilization Reporter Event Forwarder ------------------------------------------------------------------------------
comp_mg_util_reporter_forwarder = sst.Component("mg_util_reporter_event_forwarder", "QS_1_0.EventForwarder")
comp_mg_util_reporter_forwarder.addParams({
    "num_inputs": 1,
    "num_outputs": 2,
    "verbose" : verbosity_option,
})
...
# QG Utilization Reporter Event Forwarder ------------------------------------------------------------------------------
comp_qg_util_reporter_forwarder = sst.Component("qg_util_reporter_event_forwarder", "QS_1_0.EventForwarder")
comp_qg_util_reporter_forwarder.addParams({
    "num_inputs": 1,
    "num_outputs": 2,
    "verbose" : verbosity_option,
})
...
# PQ Utilization Reporter Event Forwarder ------------------------------------------------------------------------------
comp_pq_util_reporter_forwarder = sst.Component("pq_util_reporter_event_forwarder", "QS_1_0.EventForwarder")
comp_pq_util_reporter_forwarder.addParams({
    "num_inputs": 1,
    "num_outputs": 2,
    "verbose" : verbosity_option,
})
...
# LQ Utilization Reporter Event Forwarder ------------------------------------------------------------------------------
comp_lq_util_reporter_forwarder = sst.Component("lq_util_reporter_event_forwarder", "QS_1_0.EventForwarder")
comp_lq_util_reporter_forwarder.addParams({
    "num_inputs": 1,
    "num_outputs": 2,
    "verbose" : verbosity_option,
})
...
for port in ports_config:
    ...
    link_global_pool_lq.connect(
        ...
        (comp_global_pool_util, "l4s_queue_%d" % (i), "5ps"),
    )
...
# Packet Classifier --> Bytes Tracker
link_packet_classifier_bytes_tracker = sst.Link("link_packet_classifier_bytes_tracker")
link_packet_classifier_bytes_tracker.connect(
    (comp_packet_classifier, "bytes_tracker", "5ps"),
    (comp_bytes_tracker, "wred", "5ps")
)

# Packet Classifier --> Bytes Tracker
link_packet_classifier_l4s_bytes_tracker = sst.Link("link_packet_classifier_l4s_bytes_tracker")
link_packet_classifier_l4s_bytes_tracker.connect(
    (comp_packet_classifier, "l_bytes_tracker", "5ps"),
    (comp_bytes_tracker, "l4s_wred", "5ps")
)

# Packet Classifier --> Packet Buffer 
link_packet_classifier_packet_buffer = sst.Link("link_packet_classifier_packet_buffer")
link_packet_classifier_packet_buffer.connect(
    (comp_packet_classifier, "packet_buffer", "10ps"),
    (comp_pkt_buffer, "acceptance_checker", "10ps")
)

# Acceptance Checker --> L4S WRED
link_acceptance_checker_l4s_wred = sst.Link("link_acceptance_checker_l4s_wred")
link_acceptance_checker_l4s_wred.connect(
    (comp_acceptance_checker, "l4s_wred", "5ps"),
    (comp_l4s_wred, "acceptance_checker", "5ps")
)

# Acceptance Checker --> Classic WRED
link_acceptance_checker_classic_wred = sst.Link("link_acceptance_checker_classic_wred")
link_acceptance_checker_classic_wred.connect(
    (comp_acceptance_checker, "wred", "5ps"),
    (comp_classic_wred, "acceptance_checker", "5ps")
)

# Classic WRED --> L4S WRED (Coupling)
...
# Packet Classifier --> Drop Receiver Event Forwarder
link_packet_classifier_drop_receiver_event_forwarder = sst.Link("link_packet_classifier_drop_receiver_event_forwarder")
link_packet_classifier_drop_receiver_event_forwarder.connect(
    (comp_packet_classifier, "drop_receiver", "5ps"),
    (comp_drop_receiver_event_forwarder, "input_2", "5ps")
)
...
link_l4s_wred_bytes_tracker.connect(
    ...
    (comp_bytes_tracker, "l4s_wred_v2", "5ps")
)
...
link_classic_wred_bytes_tracker.connect(
    ...
    (comp_bytes_tracker, "wred_v2", "5ps")
)
...
# Egress Update ---> Packet Buffer Event Forwarder
link_egress_packet_buffer_event_forwarder = sst.Link("link_egress_packet_buffer_event_forwarder")
link_egress_packet_buffer_event_forwarder.connect( 
    (comp_egress_update, "packet_buffer", "5ps"),
    (comp_packet_buffer_event_forwarder, "input_2", "5ps"), 
)
...
link_event_forwarder_packet_buffer.connect(
    ...
    (comp_pkt_buffer, "egress_ports", "5ps")
)
...
# Utilization Reporter MG --> MG Event forwarder 
...
link_mg_reporter_acceptance_checker.connect(
    ...
    (comp_mg_util_reporter_forwarder, "input_0", "5ps"),
)

# MG Util Event Forwarder --> Acceptance Checker
link_mg_util_event_forwarder_acceptance_checker = sst.Link("link_mg_util_event_forwarder_acceptance_checker")
link_mg_util_event_forwarder_acceptance_checker.connect(
    (comp_mg_util_reporter_forwarder, "output_0", "5ps"),
    (comp_acceptance_checker, "mg_utilization", "5ps"),
)

# MG Util Event Forwarder --> Packet Classifier
link_mg_util_event_forwarder_packet_classifier = sst.Link("link_mg_util_event_forwarder_packet_classifier")
link_mg_util_event_forwarder_packet_classifier.connect(
    (comp_mg_util_reporter_forwarder, "output_1", "5ps"),
    (comp_packet_classifier, "mg_util", "5ps"),
)

# Utilization Reporter QG --> QG Event forwarder 
...
link_qg_reporter_acceptance_checker.connect(
    ...
    (comp_qg_util_reporter_forwarder, "input_0", "5ps"),
)

# QG Util Event Forwarder --> Acceptance Checker
link_qg_util_event_forwarder_acceptance_checker = sst.Link("link_qg_util_event_forwarder_acceptance_checker")
link_qg_util_event_forwarder_acceptance_checker.connect(
    (comp_qg_util_reporter_forwarder, "output_0", "5ps"),
    (comp_acceptance_checker, "qg_utilization", "5ps"),
)

# QG Util Event Forwarder --> Packet Classifier
link_qg_util_event_forwarder_packet_classifier = sst.Link("link_qg_util_event_forwarder_packet_classifier")
link_qg_util_event_forwarder_packet_classifier.connect(
    (comp_qg_util_reporter_forwarder, "output_1", "5ps"),
    (comp_packet_classifier, "qg_util", "5ps"),
)
...
link_pq_reporter_acceptance_checker.connect(
    ...
    (comp_pq_util_reporter_forwarder, "input_0", "5ps"),
)

# PQ Util Event Forwarder --> Acceptance Checker
link_pq_util_event_forwarder_acceptance_checker = sst.Link("link_pq_util_event_forwarder_acceptance_checker")
link_pq_util_event_forwarder_acceptance_checker.connect(
    (comp_pq_util_reporter_forwarder, "output_0", "5ps"),
    (comp_acceptance_checker, "pq_utilization", "5ps"),
)

# PQ Util Event Forwarder --> Packet Classifier
link_pq_util_event_forwarder_packet_classifier = sst.Link("link_pq_util_event_forwarder_packet_classifier")
link_pq_util_event_forwarder_packet_classifier.connect(
    (comp_pq_util_reporter_forwarder, "output_1", "5ps"),
    (comp_packet_classifier, "pq_util", "5ps"),
)
...
link_lq_reporter_acceptance_checker.connect(
    ...
    (comp_lq_util_reporter_forwarder, "input_0", "5ps"),
)

# LQ Util Event Forwarder --> Acceptance Checker
link_lq_util_event_forwarder_acceptance_checker = sst.Link("link_lq_util_event_forwarder_acceptance_checker")
link_lq_util_event_forwarder_acceptance_checker.connect(
    (comp_lq_util_reporter_forwarder, "output_0", "5ps"),
    (comp_acceptance_checker, "lq_utilization", "5ps"),
)

# LQ Util Event Forwarder --> Packet Classifier
link_lq_util_event_forwarder_packet_classifier = sst.Link("link_lq_util_event_forwarder_packet_classifier")
link_lq_util_event_forwarder_packet_classifier.connect(
    (comp_lq_util_reporter_forwarder, "output_1", "5ps"),
    (comp_packet_classifier, "lq_util", "5ps"),
)
# ------------------------------------------------------------------------------------------------------------
...
# Egress ports
egress_ports_receiving_traffic = [0, 8, 16, 24, 32, 40, 48, 51]
