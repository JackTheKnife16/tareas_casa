...
parser.add_argument('-w', '--weights', required=False,
                    help="Path to sheduler config file.")

parser.add_argument('-m', '--ma_config', required=False,
                    help="Path to MA config file.")
...
ma_config_file = "qs_1_0/config/ma_config/ma_config_official.json"
if args.ma_config:
    ma_config_file = args.ma_config
...
if os.path.isfile(ma_config_file):
    try:
        with open(ma_config_file) as jsonfile:
            ma_config = json.load(jsonfile)
    except Exception as e:
        logging.error("Can't parse %s" % (ma_config_file))
        exit(1)
else:
    logging.error("File %s does not exist" % (ma_config_file))
    exit(1)
...
# Scheduler weights config file
scheduler_config = "qs_1_0/config/scheduler/dwrr_config_basic.json"
if args.weights:
    scheduler_config = args.weights

# Statistics file
stat_file = "memory_accounting_basic_stats.csv"
if args.stats:
...
subcomp_traffic_profile = comp_packet_generator.setSubComponent("traffic_profile", "TRAFFIC_GEN_1_0.IngressAbsorption", 0)
...
for port in ports_config:
    ...
    comp_port_fifos.addParams({
        "port" : i,
        "verbose" : verbosity_option, ### FREDDY: CAMBIAR A 0 Y BORRAR EL COMENTARIO
    })
    comp_ports_fifos.append(comp_port_fifos)
    # --------------------------------------------------------------------------------------------------

    # SchedulerDWRR [i] ------------------------------------------------------------------------------------
    comp_scheduler = sst.Component("scheduler_%d" % (i), "QS_1_0.SchedulerDWRR")
    comp_scheduler.addParams({
        "num_queues" : 8,
        "highest_priority" : 7,
        "dwrr_config_path": scheduler_config,
        "verbose" : verbosity_option,
    })
    ...
    for j in range(8):
        ...
        # Connections -----------------------------------------------------------------------------------
        # Priority Queue[i][j] <--> SchedulerDWRR[i]
...
# Egress ports
egress_ports_receiving_traffic = [0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48]
