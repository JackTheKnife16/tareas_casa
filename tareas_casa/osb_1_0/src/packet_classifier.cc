...
/**
 * @brief Construct a new PacketClassifier::PacketClassifier object
 *
 * @param id
 * @param params
 */
PacketClassifier::PacketClassifier(ComponentId_t id, Params& params) : Component(id)
{
    // Clock frequency
    ...
    prefix << "[@t ps]:" << getName() << ":PacketClassifier[@l]: ";
    ...
    std::function<void(void)> notify = std::bind(&PacketClassifier::notify_delay_routine, this);
    ...
    delay_routine_link = configureSelfLink(
        "delay_routine", base_tc, new Event::Handler<PacketClassifier>(this, &PacketClassifier::delay_routine));
 ...
 */
void
PacketClassifier::notify_delay_routine()
{
    safe_lock.lock();
 ...
 */
void
PacketClassifier::disable_delay_routine()
{
 ...
 */
void
PacketClassifier::delay_routine(SST::Event* ev)
{
    if ( !packet_buffer->is_empty() ) {
        ...
        if ( pkt_word->start_of_packet && pkt_word->first_chunk_word ) {
            ...
            // Copying priority
            fds->priority = pkt_word->priority;
