...
/**
 * @brief Construct a new HostEgressPort::HostEgressPort object
 *
 * @param id
 * @param params
 */
HostEgressPort::HostEgressPort(ComponentId_t id, Params& params) : Component(id)
{
    // Number of nodes
    ...
    prefix << "@t:" << getName() << ":HostEgressPort[@p:@l]: ";
    out = new Output(prefix.str(), verbose, 0, SST::Output::STDOUT);

    out->verbose(CALL_INFO, MODERATE, 0, "-- Host Egress Port --\n");
    ...
    // Configure in links
    scheduler =
        configureLink("scheduler", base_tc, new Event::Handler<HostEgressPort>(this, &HostEgressPort::handle_new_pkt));
    out->verbose(CALL_INFO, MODERATE, 0, "scheduler link was configured\n");

    phys_port = configureLink("phys_port", base_tc);
    out->verbose(CALL_INFO, MODERATE, 0, "phys_port link was configured\n");
    ...
    timing_link = configureSelfLink(
        "timing_link", base_tc, new Event::Handler<HostEgressPort>(this, &HostEgressPort::output_timing));
    out->verbose(CALL_INFO, MODERATE, 0, "timing_link link was configured\n");
 ...
 */
void
HostEgressPort::output_timing(Event* ev)
{
    // Sending packet
    ...
    out->verbose(
        CALL_INFO, INFO, 0,
        "Sending pkt: pkt_id = %u, size = %dB, destination node = %d, "
        "destination port = %d, priority = %d\n",
        pkt->pkt_id, pkt->pkt_size, pkt->dest_node, pkt->dest_port, pkt->priority);

    // Requesting packet to scheduler
    scheduler->send(NULL);
    out->verbose(CALL_INFO, INFO, 0, "Requesting a new packet from the scheduler\n");

    // Sending the packet out
    phys_port->send(pkt);
    out->verbose(CALL_INFO, INFO, 0, "Sending packet out, pkt_id = %u\n", pkt->pkt_id);

    finisher->send(pkt->clone());
    out->verbose(CALL_INFO, INFO, 0, "Sending packet to finisher, pkt_id = %u\n", pkt->pkt_id);

    packet_buffer->send(pkt->clone());
    out->verbose(CALL_INFO, INFO, 0, "Sending packet to packet buffer, pkt_id = %u\n", pkt->pkt_id);
}
 ...
 */
void
HostEgressPort::handle_new_pkt(SST::Event* ev)
{
    out->verbose(CALL_INFO, INFO, 0, "Received a new event.\n");
    if ( ev == NULL ) {
        out->verbose(CALL_INFO, INFO, 0, "Received NULL event, sending NULL through scheduler link.\n");
        scheduler->send(NULL);
    }
    else {
        PacketEvent* pkt = dynamic_cast<PacketEvent*>(ev);
        if ( pkt ) {
            // Compute delay for current packet
            ...
            SST::SimTime_t send_delay = interval.getRoundedValue();
            // Schedule packet
            timing_link->send(send_delay, pkt);
            out->verbose(
                CALL_INFO, INFO, 0,
                "Packet scheduled on timing link. Packet id: %u, size: %dB, destination node: %d, destination port: "
                "%d, priority: %d, delay: %llu ps.\n",
                pkt->pkt_id, pkt->pkt_size, pkt->dest_node, pkt->dest_port, pkt->priority, send_delay);
        }
        else {
 ...
 */
void
HostEgressPort::finish()
{
    out->verbose(CALL_INFO, INFO, 0, "Egress Port %d: %d packets\n", port, sent_pkts);
}
